...
public class MITemplate {
	...
	private ArrayList<Terms> positiveTrainList;
	private ArrayList<Terms> negativeTrainList;
	...
	private ArrayList<TermMI> posResultList;
	private ArrayList<TermMI> negResultList;
	 ...
	 */
	public MITemplate(ArrayList<Terms> aPositiveTrainList,
			ArrayList<Terms> aNegativeTrainList) {
	...
	private void setup(ArrayList<Terms> aPositiveTrainList,
			ArrayList<Terms> aNegativeTrainList) {
	...
	public MITemplate(ArrayList<ArrayList<String>> aPositiveTrainList,
	...
	private void buildTermLists() {
		...
		for (int i = 0; i < positiveTrainList.size(); i++) {
			...
			Terms terms = positiveTrainList.get(i);
			ArrayList<TermPos> tpl = terms.getTermPosList();
			for (int j = 0; j < tpl.size(); j++) {
				TermPos tPos = tpl.get(j);
		...
		for (int i = 0; i < negativeTrainList.size(); i++) {
			ArrayList<String> termList = new ArrayList<String>();
			HashSet<String> termSet = new HashSet<String>();
			Terms terms = negativeTrainList.get(i);
			ArrayList<TermPos> tpl = terms.getTermPosList();
			for (int j = 0; j < tpl.size(); j++) {
				TermPos tPos = tpl.get(j);
				String term = tPos.getTerm();
	...
	private void scanTermLists() {
		...
		for (int i = 0; i < positiveTermLists.size(); i++) {
			String term = positiveTermLists.get(i);
		...
		for (int i = 0; i < negativeTermLists.size(); i++) {
			String term = negativeTermLists.get(i);
			// System.out.println(term);
	...
	private void computeValues() {
		...
		ArrayList<String> allTermsList = new ArrayList<String>();
		allTermsList.addAll(allTerms);
		for (int i = 0; i < allTermsList.size(); i++) {
			String term = allTermsList.get(i);
			double a = 0;
			if (mapA.containsKey(term)) {
				a = mapA.get(term).doubleValue();
			}
			double b = 0;
			if (mapB.containsKey(term)) {
				b = mapB.get(term).doubleValue();
			}
	...
	private class TermMI implements Comparable<TermMI> {
		public String term;
		public double value;
	...
	public static void test1() {
		...
		try {
			...
			MITemplate mi = new MITemplate(positiveList, negativeList);
	...
	public static Hashtable<listtype, Collection<String>> selectFeatures(
		...
		Hashtable<listtype, ArrayList<Terms>> lists = MITemplate.getTagLists(
				number, con, sql);
...
public void printScores(int top) {printScores(top, null,null,System.out, null);}
	...
	public void printScoresReverse(int bottom) {
		...
		List<String> olist = getTopNposResultList();
